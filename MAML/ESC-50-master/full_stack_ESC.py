...
from to_fbank import main as to_fbank_main
...
MAIN_DIR = "C:\\Users\\arsht_h67\\Desktop\\Self_Supervised_Meta_Learning\\MetaAudio-A-Few-Shot-Audio-Classification-Benchmark\\ESC-50-master\\"

# Path wated for final spectrogram data
FINAL_SPEC_PATH = MAIN_DIR + '\\ESC_fbank'
...
if __name__ == '__main__':
    ...
    # Generates a new path for the sorted classes in npy format
    sorted_npy_path = os.path.join(MAIN_DIR, sorted_path + '_npy')
    # np_norm_main_2(old_dir=sorted_path, new_dir=sorted_npy_path, sr=MEL_SPEC_PARAMS['sr'], norm=NORM)

    # Carries out the spectrogram dataste creation
    FINAL_SPEC_PATH = MAIN_DIR + '\\ESC_fbank'
    to_fbank_main(old_dir=sorted_npy_path,
                    new_dir=FINAL_SPEC_PATH,
                    sample_length=SAMPLE_LENGTH,
                    spec_params=MEL_SPEC_PARAMS)
